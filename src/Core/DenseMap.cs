...
public class DenseMap<TKey, TValue, THasher> where THasher : struct, IHasher<TKey>
{
    ...
    // twice the reach of the previous Vector128 (16-byte) groups. This halves
    // the number of probe-group iterations needed to scan the same span of
    // the table.
    ...
    private static readonly Vector256<sbyte> _emptyBucketVector = Vector256.Create(_emptyBucket);
    private static readonly Vector256<sbyte> _tombstoneVector = Vector256.Create(_tombstone);
    ...
    public DenseMap(uint length, double loadFactor)
    {
        ...
        // Padding must equal exactly one group width: a Vector256 load starting at the
        // last logical slot (index = _length - 1) reads 32 bytes and spills 31 bytes
        // past _length, so the physical array needs _length + _groupWidth bytes.
        _controlBytes = GC.AllocateArray<sbyte>((int)_length + (int)_groupWidth);
        // NOTE: entries never need the SIMD mirror padding that control bytes do -
        // every entry index is masked into [0, _length - 1]. Resize()/Rebuild() already
        // allocate entries without padding; the constructor matches that, saving
        // _groupWidth * sizeof(Entry) bytes per instance for nothing in return.
    ...
    public void Insert(TKey key, TValue value)
    {
        ...
        while (true)
        {
            ...
            var emptyMask = Vector256.Equals(source, _emptyBucketVector).ExtractMostSignificantBits();
            ...
            {
                var tombstoneMask = Vector256.Equals(source, _tombstoneVector).ExtractMostSignificantBits();
                if (tombstoneMask != 0)
                {
    ...
    /// map.InsertOrUpdate(1, "Two"); // updates value for key 1 to "Two"
    ///  
    /// </code>
    /// </summary>
    ...
    public void InsertOrUpdate(TKey key, TValue value)
    {
        ...
        while (true)
        {
            ...
            if (Vector256.Equals(source, _emptyBucketVector).ExtractMostSignificantBits() != 0)
            {
                bool wasTombstone = Unsafe.Add(ref ctrl, firstAvailableSlot) == _tombstone;
    ...
    /// <summary>
    /// Determines whether it is safe to mark a soon-to-be-removed slot as fully
    /// <c>Empty</c> instead of leaving a <c>Tombstone</c> behind.
    ///
    /// Background: probe loops only stop scanning when they hit an EMPTY control
    /// byte; a byte occupied by either a live entry OR a tombstone is treated as
    /// "keep going". That means turning a slot Empty is only safe if doing so
    /// cannot change the outcome of any OTHER key's probe: i.e. every possible
    /// group-width window that overlaps this slot already contained at least one
    /// Empty byte before this deletion. If some window overlapping this slot was
    /// ever completely full (no Empty anywhere in it), some other key's probe may
    /// have used "group has no empty -> keep probing past here" to reach a slot
    /// further along the chain; turning this slot Empty now would introduce a new,
    /// premature stop for that key's future lookups. In that case we must leave a
    /// Tombstone, which still blocks nothing but also never causes early
    /// termination.
    ///
    /// Rather than enumerate all (_groupWidth - 1) possible window positions that
    /// could overlap the slot, we measure the contiguous run of non-empty bytes
    /// straddling it using exactly two group loads (the window starting at the
    /// slot, and the window immediately preceding it). If that combined run is
    /// shorter than _groupWidth, no fully-occupied window could ever have existed
    /// there. This is the same technique Abseil's flat_hash_map uses
    /// (`was_never_full`), adapted to this table's 32-byte Vector256 groups and
    /// arbitrary (non-group-aligned) probe start offsets.
    /// </summary>
    ...
    {
        // Window starting exactly at the slot being removed: [index, index + _groupWidth).
        var after = Vector256.LoadUnsafe(ref ctrl, index);
        var emptyAfterMask = Vector256.Equals(after, _emptyBucketVector).ExtractMostSignificantBits();

        // Window immediately preceding it: [index - _groupWidth, index).
        uint beforeIndex = (index - _groupWidth) & _mask;
        var before = Vector256.LoadUnsafe(ref ctrl, beforeIndex);
        var emptyBeforeMask = Vector256.Equals(before, _emptyBucketVector).ExtractMostSignificantBits();

        // If either window currently has zero Empty bytes, we can't bound the
        // non-empty run using just these two loads (it may extend further in that
        // direction). Conservatively fall back to tombstoning - always correct,
        // just occasionally more conservative than strictly necessary.
        if (emptyAfterMask == 0 || emptyBeforeMask == 0)
        {
            return false;
        }

        // Contiguous non-empty run to the right of (and including) `index`, up to
        // the first Empty byte encountered.
        int runRight = BitOperations.TrailingZeroCount(emptyAfterMask);

        // Contiguous non-empty run ending immediately before `index`. Vector256<sbyte>
        // ExtractMostSignificantBits() yields a full 32-bit mask with every bit
        // meaningful (unlike Vector128, whose 16-bit mask would need shifting into
        // the upper bits first), so LeadingZeroCount directly gives the run length
        // ending at the boundary between the two windows.
        int runLeft = BitOperations.LeadingZeroCount(emptyBeforeMask);

        // If the combined run straddling `index` is shorter than one full group,
        // no group-width window overlapping this slot was ever entirely occupied.
        ...
        while (true)
        {
            ...
            while (matchMask != 0)
            {
                ...
                if (_hasher.Equals(entry.Key, key))
                {
                    ...
                    {
                        // No probe window touching this slot was ever fully occupied,
                        // so resetting straight to Empty can't cause any other key's
                        // lookup to stop early where it previously wouldn't have.
                        // This keeps the tombstone count down, delaying/avoiding the
                        // next Rebuild() entirely.
    ...
    public bool Contains(TKey key)
    {
        ...
        while (true)
        {
            ...
            if (Vector256.Equals(_emptyBucketVector, source).ExtractMostSignificantBits() != 0)
            {
    ...
    {
        // Iterate the source map's backing arrays directly instead of going through the
        // public Entries iterator, which allocates a compiler-generated enumerator on
        // every call. This keeps Copy() zero-allocating like the rest of the hot paths.
    ...
    private void SetCtrl(ref sbyte ctrl, uint slot, sbyte value)
    {
        ...
        // Mirror the first group into the tail padding. Only slots inside the first
        // group (indices [0, _groupWidth)) can ever be observed twice by a Vector256
        // load - once directly, once as the tail bytes of a load that wraps around
        // past _length. With _groupWidth == 32, "first group" means slot >> 5 == 0
        // (shift by 5 == divide by 32, matching the group width above).
        uint mirrorMask = (uint)-(slot >> 5 == 0 ? 1 : 0);
